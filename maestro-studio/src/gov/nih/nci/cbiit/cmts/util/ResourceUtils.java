...
public class ResourceUtils {
    ...
    * @param targetPath is calculated to this file
        ...
        */
         ...
         if(length == (normalizedTargetPath.length()+1)) { // might miss the separator
         	length = normalizedTargetPath.length();
         }
         String relPath = normalizedTargetPath.substring(length);
         relative.append(relPath);
         if(!baseIsFile && !relative.toString().endsWith(pathSeparator)) {
         	if(targetPath.endsWith(pathSeparator)) {
         		return relative.toString() + pathSeparator;
         	}
         }
