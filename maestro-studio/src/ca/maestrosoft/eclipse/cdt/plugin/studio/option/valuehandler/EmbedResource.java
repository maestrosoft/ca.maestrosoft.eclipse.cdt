...
public class EmbedResource extends RelativePath implements IManagedOptionValueHandler {
         ...
         if(configDesc != null && configDesc.getConfigurationData() != null) {
            try {
               if(!handleOption.getStringValue().equals("")) {
                  String optionValue = getRelativePath(new String[] {handleOption.getStringValue()}, "${workspace_loc}/${ProjName}/${ConfigName}", holder)[0];
                  varSupplier.createMacro("EmbedResource", ICdtVariable.VALUE_TEXT, optionValue, configDesc);
               }
            } 
