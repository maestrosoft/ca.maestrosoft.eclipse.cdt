...
public final class ToolChainVersionClass {
   ...
   public enum ToolChainVersion {
      ...
      Visual_2015("Microsoft\\VisualStudio\\SxS\\VC7", "14.0", "..\\Common7\\IDE", WindowsSDK.WinSDK_10_1.toString()); // Visual 2015
      ...
      private ToolChainVersion(String VersionKey, String VersionName, String idePath, String sdkPrefered ) {
         ...
         try {
            String localKeyPath = "SOFTWARE\\Wow6432Node\\" + toolChainVersionKey;
            if(!WindowsReg.checkReadKey(WindowsReg.HKEY_LOCAL_MACHINE, localKeyPath)) {
               localKeyPath = "SOFTWARE\\" + toolChainVersionKey;
               if(!WindowsReg.checkReadKey(WindowsReg.HKEY_LOCAL_MACHINE, localKeyPath)) {
                  localKeyPath = null;
               }
            }				
            if(localKeyPath != null) {
               ...
               if(toolchainVersionPath == null) {
                  String vcToolPath = "SOFTWARE\\Microsoft\\VisualStudio\\" + toolchainVersionName + "\\CLSID";
                  List<String> subKeys = WindowsReg.readStringSubKeys(WindowsReg.HKEY_LOCAL_MACHINE, vcToolPath);
                  for(String key : subKeys) {
                     Map<String, String> keysMap = WindowsReg.readStringValues(WindowsReg.HKEY_LOCAL_MACHINE, vcToolPath + "\\" + key);
                     String codeBaseValue = keysMap.get("CodeBase");
                     if(codeBaseValue.startsWith("file:///")) {
                        codeBaseValue = codeBaseValue.substring(8);
                        int index = codeBaseValue.indexOf("Common7");
                        if(index != -1) {
                           codeBaseValue = codeBaseValue.substring(0, index);
                           toolchainVersionPath = codeBaseValue + "VC\\";
                           break;
                        }
                     }
                  }
