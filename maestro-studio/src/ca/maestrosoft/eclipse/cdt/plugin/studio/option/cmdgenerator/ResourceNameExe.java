...
public class ResourceNameExe extends RelativePath implements IOptionCommandGenerator {
	...
	public String generateCommand(IOption option, IVariableSubstitutor macroSubstitutor) {
		...
		try {
			if(option.getValueType() == IOption.STRING) {
				return option.getCommand() + '"' + stripRelative(option.getStringValue(), macroSubstitutor) + ";#1" + '"';
			}
		} 
